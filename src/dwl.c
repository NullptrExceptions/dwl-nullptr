...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	if (!c && (node = wlr_scene_node_at(&layers[LyrBottom]->node, cursor->x, cursor->y, NULL, NULL)) &&
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
                else if (cursor->x > selmon->m.width - (20 + (int)TEXTW(selmon, stext) - selmon->lrpad)) {
                        click = ClkStatus;

                        x = selmon->m.width - (20 + (int)TEXTW(selmon, stext) - selmon->lrpad);
                        statussig = 1;

                        for(s = stext; *s && x <= cursor->x; s++)
                        {
                                ch[0] = *s;
                                if(*s == '|') {
                                        statussig++;
                                }
                                y = TEXTW(selmon, ch) - selmon->lrpad;
                                if(y != 0)
                                        x += y;
                                else
                                        x += 4;
...
drawbar(Monitor *m)
{
	...
	drwl_text(pix, m->font, m->b.width - tw, 0, tw, m->b.height, 0, stext, &normbarfg, &normbarbg);
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drwl_text(pix, m->font, x, 0, w, m->b.height, pad/2, tags[i],
